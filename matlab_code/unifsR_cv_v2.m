function [CV_model, CV_fs, CV_fs1, acctab, score, foldid, time0, CV_alphalist, CV_Dlist, unifs_cv] = unifsR_cv_v2(train, test, cp, branch, parlist)
%% Checking Input Data %%
...
fprintf('fold %2d:\tD = %8.3f\n', [rem(x-1, kfold)+1; unifs_cv{x}{5}(3)]);
    ...
    R_list = zeros(kalpha,1);
    D_list = zeros(kalpha,1);
    convexD = unifs_cv{x}{5}(3) * ones(size(unifs_cv{x}{2}));

    % Generate convex-hull constrain: convexD1 based on model.C1
    [model, cv_dec, ~] = unifsR_lin(unifs_cv{x}{1}, unifs_cv{x}{2}, unifs_cv{x}{3}, branch, convexD);
    model.label = label;
    score_list(end) = {cv_dec};
    model_list(end) = {model};
    alpha_list(end) = model.alpha;
    ...
    [model, cv_dec, ~] = unifsR_con(unifs_cv{x}{1}, unifs_cv{x}{2}, unifs_cv{x}{3}, branch, branch, 0, convexD);
    model.label = label;
    score_list(1) = {cv_dec};
    model_list(1) = {model};
    alpha_list(1) = model.alpha;
