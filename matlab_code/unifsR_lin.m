function [model, score, alpha] = unifsR_lin(TRAIN, y, TEST, beta1, convexD)
...
b_eq = [200; 0];
A_ineq = [[diag(beta1)*E'; -diag(beta1)*E'],-ones(2*np,1)];
b_ineq = zeros(2*np,1);

%% Matlab linprog linear solver %%
% options = optimset('Display','off','Algorithm','Active-set');
% [u, ~, ~] = linprog([zeros(npar,1);1], A_ineq, b_ineq, A_eq, b_eq, lD, [uD;Inf], [], options);
%% TOMLAB linear programming %%
Prob = lpAssign(sparse(npar+1,1,1), [A_ineq; A_eq], [-Inf(size(b_ineq));b_eq], [b_ineq;b_eq], lD, [uD;Inf], [], 'linear');
...
% [u, ~, ~] = snopt(zeros(npar+1,1), lD, [uD;Inf], [0;-Inf(size(b_ineq));b_eq], [Inf;b_ineq;b_eq], 'uf', A(:), iAfun(:), jAvar(:), [], []);
...
A_mat = full([(y+1)/2, (y-1)/2, -E(:,rescale)]);
b_mat = zeros(npar,1);
