...
%% Read in Meta Info %%
metainfo = csvimport('./0.raw/HMP/v13_map_uniquebyPSN.txt', 'delimiter', '\t');
metaInfo = struct;
...
taxainfo = csvimport('./0.raw/HMP/otu_taxa_psn_v13.csv', 'delimiter', ',');
taxaInfo = struct;
...
%% Read in Data Info %%
spa = spconvert(load('./0.raw/HMP/otu_table_psn_v13.sp.mt'));
dataInfo = full(spa);
rownames = csvimport('./0.raw/HMP/otu_table_psn_v13.sp.rn', 'noHeader', true);
colnames = csvimport('./0.raw/HMP/otu_table_psn_v13.sp.cn', 'noHeader', true);
...
HMPv13 = phyloseq('otu_table', dataInfo, 'sam_table', metaInfo, 'phy_tree', './0.raw/HMP/rep_set_v13.tre', 'tax_table', taxaInfo);
save('./0.hmp.input/HMPv13.phylo.mat', 'HMPv13', '-v7.3');

%% Generate Select Groups for Validation %%
load('./0.hmp.input/HMPv13.phylo.mat');
% label = {groupinfo.Group1{19}, groupinfo.Group2{19}};
% parlist.size = [groupinfo.G1_v13_train(c); groupinfo.G2_v13_train(c)];
label = {'Subgingival_plaque', 'Supragingival_plaque'};
parlist.size = [120;120];
parlist.trim = [1e-3;1e-3;1000];
cvp = arrayfun(@(x) cv_partition(HMPv13, 'HMPbodysubsite', label, parlist), 1:50, 'Uniformoutput', false);
parlist.size = [1;1];
parlist.trim = [1e-3;1e-3;0];
cvp{51} = cv_partition(HMPv13, 'HMPbodysubsite', label, parlist);
save('./0.hmp.input/HMPv13.c19.cvp.mat', 'cvp');
